...
Generated at commit `533d33794379df9790b63e2e3c3938e52afaa404` on branch `feat/worker-tls`.

Falcoria: a network-scanning platform, uv-workspace monorepo, three deployable services
(`scanledger`, `tasker`, `worker`) plus four shared libraries. `README.md`/`AGENTS.md`
describe `tasker`/`worker` as "not created yet" — that is stale; only `falcli` is actually
unbuilt. See `ARCHITECTURE.md` for detail.
...
- `README.md`/`AGENTS.md` claim `tasker`/`worker`/`falcli` are "not created yet"; `tasker` and
  `worker` are implemented and deployed. `[ASK USER]` whether to correct those two files —
  this skill does not edit files outside `docs/`.
